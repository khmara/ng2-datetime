...
@Component({
    ...
    `,
    styles: [
        '.ng2-datetime *[hidden] { display: none; } .form-control[readonly] { background: #fff; }'
    ]
})
...
export class NKDatetime implements ControlValueAccessor, AfterViewInit, OnDestroy, OnChanges {
    ...
    @Input('onInput') onInputCallback: any;
    @Input('model') parentModel: any;
    ...
    onInput ( e: any ) {
        ...
        if ( this.onInputCallback && typeof this.onInputCallback == 'function' ) this.onInputCallback ( e.target.value, this.parentModel );
    }
